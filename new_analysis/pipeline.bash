...
# If you want to use this, you probably need to change a lot in this file
...
# but for large ammount of genomes snakemake is probably a good choice
...
join -t $'\t' -a{1,2} -e "NULL" -o 0 1.{4..7} 2.{4..6} 2.9 combinetypes/types.tsv.keyed combinetypes/spacers.info.tsv.keyed

mtawk '{print $1"@"$2,$0}' ../parsemacsyfinder/types.tsv | sort -k1,1 --parallel=30 | join -a{1,2} -e "NULL" -t $'\t' -o 0 1.{4..7} 2.{4..6} 2.9 1.8 - <( mtawk '{print $1"@"$2,$0}' ../parsecrisprdetect/spacers.info.tsv | sort -k1,1 --parallel=30 )
...
#     python3 ~/blast_selfhits/align_flanks.py
...
join -t $'\t' <( mtawk '{print $1"@"$2}' ../combine_virsorter/virsorter.unique.combined.tsv |
    sort -k1,1 --parallel=70 ) \
    <( mtawk '{gene=$1; sub("_[^_]*$","",gene); print $1"@"gene,$0;}' gene_classification |
    sort -k1,1 --parallel=70 ) |
    python3 ~/new_analysis/print_gene_match.py > gene.matched.tsv
