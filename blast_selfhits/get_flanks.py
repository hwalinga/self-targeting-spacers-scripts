...
ap.add_argument(
    "-g", "--genome-path", default="/hosts/linuxhome/mgx/DB/PATRIC/patricdb-201*/"
)
...
synced_out = partial(print, file=args.out, sep=args.field_seperator, flush=True)
...
for F in csv.reader(fileinput.input(args.selfhits), delimiter=args.field_seperator):
...
def process_selfhit_item(selfhit_item, out=synced_out):
    ...
    for record in SeqIO.parse(genome_file, "fasta"):
        for hit in v[record.id]:
            ...
            end_record = len(seq) if end_record > len(seq) else end_record
            three_prime = seq[end : end + flank]
            if direction == "Reverse":
                five_prime, three_prime = (
                    three_prime.reverse_complement(),
...
if args.max_procs == 1:
    for selfhit in selfhits.items():
        process_selfhit_item(
            selfhit, out=partial(print, file=args.out, sep=args.field_seperator)
        )
else:
    if args.max_procs == 0:
