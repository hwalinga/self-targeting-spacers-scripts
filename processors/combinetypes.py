...
ap.add_argument("--genome", type=argparse.FileType("a+"), default="genome.types.tsv")
ap.add_argument("--crispr", type=argparse.FileType("a+"), default="crispr.types.tsv")
args = ap.parse_args()
...
def crisprrepeat2castype(crisprrepeat):
    ...
    return frozenset("CAS-Type" + majorType + sub for sub in subType.split("/"))


classified_repeats = {
    item for line in args.found_repeats for item in crisprrepeat2castype(line)
}
...
def say_one_type(list_of_types):
    ...
    if not no_single_cas:
        return "CAS"
    typed, subtypes = map(
        set, zip(*[megasplit(r"(?<=V|I)(?!V|I)", s) for s in no_single_cas])
    )
    if len(typed) == 1:
        if len(subtypes) > 3:
            return typed.pop()
...
def combine_types(list_of_types):
    ...
    if not list_of_types:
        return "NA"
    types_list = {s for t in list_of_types for s in t.split("|")}
    types_list_iter = map(lambda x: megasplit(r"(?<=V|I)(?!V|I)", x), types_list)
    first = next(types_list_iter)
    return first[0] + "/".join(
        sorted(set([first[1]] + list(map(itemgetter(1), types_list_iter))))
    )
...
def determine_cas(cas, crispr, cas2crispr_links):
    ...
    cas_loci = [
        determine_one_caslocus(c, crispr, cas2crispr_links) for c in cas.items()
    ]
    ...
    if not cas_loci_single:
        ...
        if confirmed:
            ...
            return (
                say_one_type(map(itemgetter(0), confirmed)),
                "UnclassifiedConfirmed" + Extra,
                amm_genes,
            )
        return (
            say_one_type(map(itemgetter(0), cas_loci_extra)),
            "Unclassified",
            amm_genes,
        )
    ...
    if not confirmed:
        ...
        if len(non_confirmed) == 1 or ("/" not in the_type and the_type != "CAS"):
            return (
                non_confirmed.pop()[0],
                Accompanied + "SingleUnknown" + Extra,
                amm_genes,
            )
    ...
    if close:
        ...
        if len(confirmed) == 1 or ("/" not in the_type and the_type != "CAS"):
            consensus_type = combine_types(
                map(itemgetter(0), chain(confirmed, non_confirmed))
            )
            if len(confirmed) + len(non_confirmed) == 1 or (
                ...
                return (
                    close.pop()[0],
                    Accompanied + "SingleConfirmed" + Extra,
                    amm_genes,
                )
            return (
                close.pop()[0],
                Accompanied + "MultipleSingleConfirmed" + Extra,
                amm_genes,
            )
    ...
    if len(distantly) == 1:
        ...
        return (
            close.pop()[0],
            Accompanied + "SingleDistantlyConfirmed" + Extra,
            amm_genes,
        )

    # implicit `len(comfirmed) > 1`
    return (
        combine_types(map(itemgetter(0), confirmed)),
        Accompanied + "MultipleDistantlyConfirmed" + Extra,
        amm_genes,
    )
...
def determine_crispr(crispr, cas, crispr2cas_links, genome_type):
    ...
    for crispr_item in crispr.items():
        res.append(
            determine_one_crispr(crispr_item, cas, crispr2cas_links, genome_type)
        )
    return res
...
def print_results(res, genome):
    ...
    cas = {
        (F[0], F[3], F[4]): (F[1], F[2], F[-1].count("|") + 1)
        for F in res
        if F[1] != "NULL"
    }
    ...
    for c in determine_crispr(crispr, cas, crispr2cas_links, (castype, casclass)):
        crispr_out(genome, combine_types(crisprrepeat2castype(c[0])), c[1], c[2])
...
for F in csv.reader(fileinput.input(args.inputfiles), delimiter=args.sep):
    ...
    if genome != last_genome:
        ...
        last_genome = genome
    F[3], F[4], F[6], F[7] = map(
        lambda x: "NULL" if x == "NULL" else int(x), [F[3], F[4], F[6], F[7]]
    )
    results.append(F)
