...
ap.add_argument("inputfiles", nargs="*", help="macsyfinder.out files")
...
class ProdiagalPrinter:
    ...
    def match_genes(self):
        ...
        for job in self.jobs:
            contigs = ["_".join(contig_id.split("_")[:-1]) for contig_id in job[3]]
            most_common_contig = Counter(contigs).most_common(1)[0][0]
            job[3] = [
                contig_id
                for contig_id, contig in zip(job[3], contigs)
        ...
        for rec in GFF.parse(prodigal_file):
            for job in contig_dict[rec.id]:
                ...
                for contig_id in job[3]:
                    loc = rec.features[int(contig_id.split("_")[-1]) - 1].location
...
def analyse_clusters(f, disambiguation=False):
    ...
    for line in f:
        ...
        if line.startswith("--- Cluster"):
            ...
            crispr_type = [re.search(r"--- Cluster\s(.*)\s\??\s---", line).group(1)]
            ...
            while considering:
                crispr_condition.append(
                    re.search(r"->\s(.*)(?:\s|$)", next(f)).group(1)
                )
                line = next(f)
            ...
            if ambiguous is True:
                ...
                clusters[genes_number] = (
                    "|".join(crispr_types),
                    "ambiguous" if len(crispr_types) > 1 else crispr_condition[-1],
                    genes_pos,
                    genes_type,
                )
            ...
            if line.startswith("Disambiguation"):
                ...
                if flag:
                    ...
                    log("Odd disambiguation case (unexpected next line):", genome)
                if not children:
        ...
        if line.startswith("Counter("):
            for genes_num, cluster in clusters.items():
                ...
                disambiguation_clusters_nums = disambiguation_links.get(genes_num)
                ...
                disambiguation_clusters = {
                    ...
                    if k in disambiguation_clusters_nums
                }
                if len(disambiguation_clusters) == 1:
                    ...
                    out(
                        genome,
                        "|".join(cluster_types),
                        "ambiguous" if len(cluster_types) > 1 else children_cluster[1],
                        cluster[2],
                        cluster[3],
                    )
                    continue
                ...
                for (
                    ...
                    if len(has_intersection) == 1:
                        print_jobs.append([genome, *disambiguation_cluster])
                        printed.update(has_intersection)
                        total_printed += 1
                    else:
                        intersection_clusters = [
                            disambiguation_clusters[num] for num in has_intersection
                        ]
                        crispr_type = "|".join(
                            map(itemgetter(0), intersection_clusters)
                        )
                        genes_pos = set().union(
                            map(set, map(itemgetter(2), intersection_clusters))
                        )
                        genes_type = set().union(
                            map(set, map(itemgetter(3), intersection_clusters))
                        )
                        print_jobs.append(
                            [genome, crispr_type, "ambiguous", genes_pos, genes_type]
                        )
                        printed.update(has_intersection)
                        total_printed += 1
                if total_printed == 1:  # Still combine with parent.
                    ...
                    out(
                        genome,
                        "|".join(cluster_types),
                        "ambiguous" if len(cluster_types) > 1 else print_jobs[0][2],
                        cluster[2],
                        cluster[3],
                    )
                else:
