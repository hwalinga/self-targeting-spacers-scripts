        ...
        for ind, val in contigs_dict.items():
            if len(val) > 0:
                ...
                inframe = "|".join(
                    map(
                        reduce(
                            lambda f, g: lambda x: g(f(x)), [itemgetter(1), int, str]
                        ),
                        val,
                    )
                )
...
with open(INPUT_FILE) as f:
    for line in f:
        ...
        spacer_start, spacer_end = map(
            lambda x: int(x.split(".")[0]), splitted_line[6:8]
        )
        ...
        for ind, (s, e) in enumerate(contig_coords):
            ...
            if minimum <= spacer_start <= maximum or minimum <= spacer_start <= maximum:
